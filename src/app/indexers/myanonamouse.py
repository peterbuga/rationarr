...
class Myanonamouse(BaseIndexer):
    ...
    async def extract_info(self):
        ...
        # save latest cookies
        await self.save_cookies(response.cookies or self.client.cookies)
            ...
            scrapers.append(
                Scraper(
                    **{
                        ...
                        "attribute": getattr(MyanonamouseScraperFields, key),
                        "value": value,
                    }
                )
            )
        ...
        response = await self.client.get(my_info_url)
